...
**Verdict.** **Does not survive, on the commodity sleeve, after publication.** All four pre-registered kill criteria fired. Over 193 months from 2010-07-31 to 2026-08-31 on seven commodity futures, the rule as specified returns +1.86% a year at 8.6% volatility — a Sharpe of +0.22 whose bootstrap interval [-0.17, +0.59] contains zero. The decisive test is not that number but K4: replacing the momentum signal with coin flips, while keeping the position sizes the rule chose, places the strategy above 76% of 200 random draws. On this evidence the momentum signal is not distinguishable from random signs under the tested sizing rule.
...
| | Sharpe | annualised |
|---|---|---|
| **time-series momentum** | **+0.22** | **+1.86%** |
| passive long, same sizing | +0.10 | +0.99% |
...
Regressing the strategy on the two economic benchmarks with HAC standard errors leaves an alpha of +1.60% a year with t = +0.96 over 193 months. Its annualised 95% interval is [-1.65%, +4.85%]; it contains zero, so K1 fires.

The loadings are more interesting than the alpha. Cross-sectional momentum takes a coefficient of +0.957 with t = +4.79: the two rules move together closely. The passive long takes -0.099 with t = -0.62 — essentially nothing.
...
This is the criterion that reads backwards: survival is the death sentence. Each of the 200 draws keeps the strategy's position *magnitudes* — the inverse-volatility scaling, the cap, the equal weighting — and replaces only the signs with fair coin flips. The draws land at a mean Sharpe of +0.03 with a 5–95 band of [-0.39, +0.47]. The strategy's +0.22 sits above 76% of them — inside the band, not beyond it.
...
| | annualised | Sharpe |
|---|---|---|
| common direction (net long/short) | +1.87% | — |
| cross-market selection | -0.01% | -0.00 |

Everything the rule earned came from being net long or net short at the right times; deciding which of the seven markets to favour contributed -0.01% a year. And the contributions are not broadly spread: 2 of the 7 markets contributed negatively, with ZS at -1.34% a year.
...
An earlier, separately written backtest of the same construction — single 12-month signal, inverse-volatility sizing, equal weight, scaled to a 15% portfolio volatility target, the same seven QuantConnect markets from 2009 — reported a Sharpe of +0.11. This run gets +0.22.
...
Three things together, all fixed in advance: an alpha in the spanning regression with |t| > 2 rather than the +0.96 observed; the strategy above the 95th percentile of the sign-randomised draws rather than above only 76% of them; and a Sharpe whose sign held across the pre-registered lookback and volatility-window sweeps. Any one of the three alone would have been suggestive; the protocol required all three because each of them has a well-known way of being produced by accident.
